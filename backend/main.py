...
def run_snake_pattern_analysis(mask, shape_name="generated_shape"):
    ...
    result = visualize_snake_pattern(mask, snake_paths, output_filename, pink_branches, connector_branches)
    
    # Save line coordinates to JSON for 300x300 grid after visualization
    print("💾 Saving line coordinates to JSON...")
    json_data = save_line_coordinates_to_json(snake_paths, scaffold_array, shape_name, json_filename, target_size=300, pink_branches=pink_branches, connector_branches=connector_branches)
    ...
    print(f"   🔗 Cyan C-Connector Pixels: {json_data['summary']['total_cyan_pixels']:,}")
